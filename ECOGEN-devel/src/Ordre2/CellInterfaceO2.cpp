...
{
  if (m_boundariesChildren.size() == 0) {
...
void CellInterfaceO2::solveRiemann(const int &numberPhases, const int &numberTransports, double &dtMax, Limiter &globalLimiter, Limiter &interfaceLimiter, Limiter &globalVolumeFractionLimiter, Limiter &interfaceVolumeFractionLimiter, double m_physicalTime, Prim type)
{
  ...
  if (globalVolumeFractionLimiter.AmITHINC() || interfaceVolumeFractionLimiter.AmITHINC()) {
    if ((alphaCellLeft >= epsInterface) && (alphaCellLeft <= 1. - epsInterface) && ((alphaCellRight - alphaCellLeft)*(alphaCellLeft - alphaCellLeftLeft) >= 1.e-8)) {
      ...
      qmin = min(alphaCellRight, alphaCellLeftLeft);
      qmax = max(alphaCellRight, alphaCellLeftLeft) - qmin;
  ...
  if (globalVolumeFractionLimiter.AmITHINC() || interfaceVolumeFractionLimiter.AmITHINC()) {
    if ((alphaCellRight >= epsInterface) && (alphaCellRight <= 1. - epsInterface) && ((alphaCellRightRight - alphaCellRight)*(alphaCellRight - alphaCellLeft) >= 1.e-8)) {
      ...
      qmin = min(alphaCellRightRight, alphaCellLeft);
      qmax = max(alphaCellRightRight, alphaCellLeft) - qmin;
...
void CellInterfaceO2::creerBordChild()
{
  m_boundariesChildren.push_back(new CellInterfaceO2(m_lvl + 1));
}

//***********************************************************************

void CellInterfaceO2::creerBordChildInterne(const int &lvl, vector<CellInterface*> *childrenInternalBoundaries)
{
  (*childrenInternalBoundaries).push_back(new CellInterfaceO2(lvl + 1));
}
